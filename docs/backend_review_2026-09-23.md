...
> 整改状态：**22 项已落地**（第二节）+ 3 条产品决策已执行（第三节），全部改动均补回归测试并通过门禁
...
- 本轮已落地：**22 项修复**（第二节）+ **3 条产品决策**（第三节）+ 2 个数据迁移脚本；新增回归测试 154 例（守卫矩阵 72、导入/审计 8、认证与并发 22、统计锁行为 3、分层/上限/日志 12、本轮其余修复 37）；随排行榜下线移除 11 例。
...
./.venv/bin/python -m pytest -q               # 606 passed（新增 154 例、随排行下线移除 11 例；整改前 463）

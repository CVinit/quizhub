...
## 八、补记（2026-09-24 复查）：第二节「仅 5 项未落地」的结论不成立

第 23 项（第二节）用「文件是否被本次改动过」筛选候选，**粒度到文件**：只要某文件因其它
原因被改动过，其中的结论就被默认视为已修复。2026-09-24 逐条读码 + 实跑复查发现以下条目
实际未落地：

| 条目 | 位置 | 实际状态（2026-09-24 复查） |
| --- | --- | --- |
| 4.2 P1#3 | `user_service.create_user` / `auth_service.register` 的 check-then-insert | 未修复：并发同邮箱仍抛 IntegrityError → 500 |
| 4.2 P1#6 | `group_service._strip_group_from_assignments` 全表加载 | 未修复；且该函数会把「仅指派给被删分组」的考试清成 `[]`，而空指派 = 全员可见 → **删除分组即把该考试对所有人开放**（新发现的 fail-open，已实跑复现） |
| 4.2 P1#7 | `user_service.py` 体积超标 | 未修复（仍 631 行） |
| 4.2 P1#18 | `max_questions_per_exam` 死配置 | 未修复（管理端仍展示该无效开关） |
| 4.2 P1#21 | 判断题布尔/大写单元格被拒 | 未修复（已实跑复现） |
| 4.2 P1#22 | 填空题空位数不交叉校验 | 未修复（已实跑复现） |
| 4.2 P1#23 | 拖拽题重复左项被静默覆盖 | 未修复（已实跑复现） |

**改进后的复查判据（下次沿用）**：每条结论必须指出「修复代码所在的具体行」，而不是
「文件被改动过」。仅凭文件级 diff 判断，会把「在同一文件里改了别的问题」误判为已修复。

上述条目已于 2026-09-24 轮次全部修复，另附该轮新发现（含验证码可离线反查、用户导入模板
示例行可导入等），见 [backend_review_2026-09-24.md](backend_review_2026-09-24.md)。

...
from app.schemas.question import (
    QuestionBankCreate, QuestionCreate, QuestionUpdate,
)
...
def list_banks(db: Session) -> list[dict]:
    ...
    rows = db.execute(
        select(QuestionBank).order_by(QuestionBank.id.desc())
    ).scalars().all()
    if not rows:
        return []
    ...
    return [
        {"id": b.id, "name": b.name, "group_id": b.group_id,
...
def list_questions(
    ...
    type_: str | None = None, bank_id: int | None = None,
    group_id: int | None = None, difficulty: int | None = None,
    ...
    for name in tags:
        ...
        if not exists:
            try:
                ...
                # 并发或重复时忽略（SAVEPOINT 已回滚，主事务不受影响）
                pass
...
def create_question(db: Session, payload: QuestionCreate) -> Question:
    ...
    q = Question(
        bank_id=payload.bank_id, type=payload.type, question=payload.question,
        options=payload.options, left_items=payload.left_items, right_items=payload.right_items,
        answer=payload.answer, analysis=payload.analysis, difficulty=payload.difficulty,
        tags=payload.tags, score=payload.score, group_id=payload.group_id,
    )
    db.add(q)
    db.commit()
    db.refresh(q)

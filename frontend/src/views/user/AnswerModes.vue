<template>
  <div class="answer-modes">
    <div class="modes-header">
      ...
      <div class="range-picker">
        ...
        <el-select v-model="selectedBank" placeholder="全部题库" style="width: 280px" @change="onRangeChange">
          <el-option :value="null" :label="`全部题库（${totalCount} 题）`" />
    ...
    <div class="modes" v-loading="loading">
      <div class="mode-card" v-for="m in modeList" :key="m.key" @click="onPick(m.key)">
    ...
    <el-dialog v-model="typeDialog" title="选择题型" width="320px">
      <div class="type-list">
        <el-button v-for="t in types" :key="t" @click="startType(t)">
          {{ t }}（{{ typeDist[t] || 0 }} 题）
        </el-button>
      </div>
    </el-dialog>
...
// selectedBank: null = 全部题库（总题库）；number = 选定某题库
const selectedBank = ref<number | null>(null)
const counts = ref({ total: 0, practiced: 0, wrong: 0, marked: 0 })

const modeList = ref([
  { key: 'sequence', title: '顺序练习', desc: '按题目顺序逐题作答', icon: Document, count: undefined as number | undefined },
...
const load = async () => {
  loading.value = true
  try {
    // 传 selectedBank 范围过滤；接口返回的 banks 始终全量
    ...
    modeList.value[3].count = d.wrong
    modeList.value[4].count = d.marked
  } finally {
    loading.value = false
  }
}
...
const onPick = (key: string) => {
  ...
  if (selectedBank.value != null) query.bank = String(selectedBank.value)
...
<style scoped>
.answer-modes { max-width: 900px; }
.modes-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px; }
.answer-modes h2 { margin: 0; }
.range-picker { display: flex; align-items: center; gap: 10px; }
.range-label { color: #606266; font-size: 14px; }
.modes { display: grid; grid-template-columns: repeat(auto-fill, minmax(220px, 1fr)); gap: 16px; }
.mode-card { background: #fff; border-radius: 8px; padding: 24px; cursor: pointer; transition: all .2s; border: 1px solid #ebeef5; text-align: center; }
.mode-card:hover { box-shadow: 0 4px 12px rgba(0,0,0,.08); transform: translateY(-2px); }
.mode-icon { font-size: 32px; color: var(--brand-primary); margin-bottom: 12px; }
.mode-title { font-weight: 600; font-size: 16px; margin-bottom: 4px; }
.mode-desc { color: #909399; font-size: 13px; }
.mode-count { color: var(--brand-primary); font-size: 13px; margin-top: 8px; }
.type-list { display: flex; flex-direction: column; gap: 8px; }
.type-list .el-button { margin-left: 0; }
@media (max-width: 767px) {
  .modes-header { flex-direction: column; align-items: stretch; gap: 12px; }
  .range-picker .el-select { width: 100% !important; }
  .modes { grid-template-columns: repeat(2, 1fr); gap: 10px; }

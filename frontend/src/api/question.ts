import { api, downloadBlob } from '@/api/http'
import type { QuestionAnswer, QuestionOptions, QuestionPairItems } from '@/api/practice'

export interface QuestionBank {
  id: number
  name: string
  group_id: number | null
  question_count?: number
  /** 是否开放给用户练习（题库管理开关） */
  practice_enabled?: boolean
}

export interface QuestionItem {
  id: number
  bank_id: number | null
  type: string
  question: string
  options: QuestionOptions
  left_items: QuestionPairItems
  right_items: QuestionPairItems
  answer: QuestionAnswer
  analysis: string
  difficulty: number
  tags: string[] | null
  score: number
  group_id: number | null
}

/** 题目列表查询参数（空条件由调用方省略）。 */
export interface QuestionListParams {
  page?: number
  page_size?: number
  keyword?: string
  bank_id?: number
  type?: string
  difficulty?: number
}

export interface QuestionPage {
  total: number
  page: number
  page_size: number
  items: QuestionItem[]
}

/** Excel 上传预览的一行（由解析器按行填充）。 */
export interface UploadPreviewRow {
  row_index: number
  type: string
  question: string
  answer: QuestionAnswer
  valid: boolean
  error?: string
}

export interface UploadPreview {
  rows: UploadPreviewRow[]
  total: number
  valid_count: number
  type_dist: Record<string, number>
  errors: { sheet?: string; row: number; error: string }[]
  confirm_token: string
  /** 超出单次导入行数上限被截断时为 true（后端返回） */
  truncated?: boolean
}

/**
 * 题目写入载荷：与后端 QuestionCreate/QuestionUpdate（均为 extra="forbid"）逐字段对齐。
 *
 * 此处刻意不用 `Partial<QuestionItem>`：后者含 `id`，一旦把列表项整个 spread 进请求，
 * 后端会因 Extra inputs are not permitted 返回 422。
 */
export interface QuestionWritePayload {
  bank_id?: number | null
  type?: string
  question?: string
  options?: string[] | null
  left_items?: string[] | null
  right_items?: string[] | null
  answer?: QuestionAnswer
  analysis?: string
  difficulty?: number
  tags?: string[] | null
  score?: number
  group_id?: number | null
}

export const questionApi = {
  // practice_enabled 省略时返回全部题库（其他管理页选题库用，需包含已关闭的）
  listBanks: (practice_enabled?: boolean) =>
    api.get<QuestionBank[]>('/admin/question-banks', {
      params: practice_enabled === undefined ? {} : { practice_enabled },
    }),
  createBank: (name: string, group_id?: number | null) => api.post('/admin/question-banks', { name, group_id }),
  // 更新题库：改名 / 练习开关（practice_enabled）
  updateBank: (id: number, data: { name?: string; practice_enabled?: boolean }) =>
    api.put(`/admin/question-banks/${id}`, data),
  deleteBank: (id: number) => api.delete(`/admin/question-banks/${id}`),
  list: (params: QuestionListParams) => api.get<QuestionPage>('/admin/questions', { params }),
  create: (data: QuestionWritePayload) => api.post('/admin/questions', data),
  update: (id: number, data: QuestionWritePayload) => api.put(`/admin/questions/${id}`, data),
  remove: (id: number) => api.delete(`/admin/questions/${id}`),
  // 按组卷来源条件统计各题型可用题量（供题型配比编辑提示）
  typeStats: (sources: { bank_ids?: number[]; group_ids?: number[]; tags?: string[] }) =>
    api.get<Record<string, number>>('/admin/question-type-stats', {
      params: {
        bank_ids: (sources.bank_ids || []).join(','),
        group_ids: (sources.group_ids || []).join(','),
        tags: (sources.tags || []).join(','),
      },
    }),
}

export const uploadApi = {
  // 模板下载需带鉴权头（window.open 无法携带 Authorization），统一走 downloadBlob
  downloadTemplate: () => downloadBlob('/admin/upload/template', '题库导入模板.xlsx'),
  preview: (file: File, group_id?: number | null, bank_id?: number | null, bank_name?: string) => {
    const fd = new FormData()
    fd.append('file', file)
    if (group_id) fd.append('group_id', String(group_id))
    if (bank_id) fd.append('bank_id', String(bank_id))
    if (bank_name) fd.append('bank_name', bank_name)
    return api.post<UploadPreview>('/admin/upload/preview', fd)
  },
  doImport: (confirm_token: string) => {
    const fd = new FormData()
    fd.append('confirm_token', confirm_token)
    return api.post<{ success: number; failed: number }>('/admin/upload/import', fd)
  },
}
